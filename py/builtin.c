...
STATIC mp_obj_t mp_builtin_max(uint n_args, const mp_obj_t *args) {
    if (n_args == 1) {
        ...
        mp_obj_t item;
        while ((item = mp_iternext(iterable)) != MP_OBJ_STOP_ITERATION) {
            if (max_obj == NULL || mp_obj_is_true(mp_binary_op(MP_BINARY_OP_LESS, max_obj, item))) {
                max_obj = item;
            }
        }
        if (max_obj == NULL) {
        ...
        for (int i = 1; i < n_args; i++) {
            if (mp_obj_is_true(mp_binary_op(MP_BINARY_OP_LESS, max_obj, args[i]))) {
                max_obj = args[i];
            }
        }
        return max_obj;
...
STATIC mp_obj_t mp_builtin_min(uint n_args, const mp_obj_t *args) {
    if (n_args == 1) {
        ...
        mp_obj_t item;
        while ((item = mp_iternext(iterable)) != MP_OBJ_STOP_ITERATION) {
            if (min_obj == NULL || mp_obj_is_true(mp_binary_op(MP_BINARY_OP_LESS, item, min_obj))) {
                min_obj = item;
            }
        }
        if (min_obj == NULL) {
        ...
        for (int i = 1; i < n_args; i++) {
            if (mp_obj_is_true(mp_binary_op(MP_BINARY_OP_LESS, args[i], min_obj))) {
                min_obj = args[i];
            }
        }
        return min_obj;
