...
set(MICROPY_QSTRDEFS_LAST_IN "${MICROPY_GENHDR_DIR}/qstr.i.last.in")
...
set(MICROPY_QSTRDEFS_PREPROCESSED_IN "${MICROPY_GENHDR_DIR}/qstrdefs.preprocessed.in.h")
...
file(GENERATE
  OUTPUT ${MICROPY_QSTRDEFS_LAST_IN}
  CONTENT "${MICROPY_CPP_FLAGS}"
)
...
add_custom_command(
    ...
    COMMAND  sed -i "s/\;/\\n/g" ${MICROPY_QSTRDEFS_LAST_IN}
    COMMAND  sed -i [[s/ /\\ /g]] ${MICROPY_QSTRDEFS_LAST_IN}
    COMMAND  sed -i [[s/\"/\\"/g]] ${MICROPY_QSTRDEFS_LAST_IN}
    COMMAND ${Python3_EXECUTABLE} ${MICROPY_PY_DIR}/makeqstrdefs.py pp ${CMAKE_C_COMPILER} @${MICROPY_QSTRDEFS_LAST_IN} -E output ${MICROPY_GENHDR_DIR}/qstr.i.last cflags -DNO_QSTR  sources ${MICROPY_SOURCE_QSTR}
    DEPENDS ${MICROPY_MPVERSION}
        ${MICROPY_SOURCE_QSTR}
	${MICROPY_QSTRDEFS_LAST_IN}
    VERBATIM
...
add_custom_command(
    ...
    COMMAND cat ${MICROPY_QSTRDEFS_PY} ${MICROPY_QSTRDEFS_PORT} ${MICROPY_QSTRDEFS_COLLECTED} | sed "s/^Q(.*)/'&'/" > ${MICROPY_QSTRDEFS_PREPROCESSED_IN}
    COMMAND ${CMAKE_C_COMPILER} @${MICROPY_QSTRDEFS_LAST_IN} -E ${MICROPY_QSTRDEFS_PREPROCESSED_IN} | sed "s/^'\\(Q(.*)\\)'/\\1/" > ${MICROPY_QSTRDEFS_PREPROCESSED}
