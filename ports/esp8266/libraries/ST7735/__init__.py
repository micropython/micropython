...
class TFT(object):
	...
	def __init__(self, spi, aDC, aReset, aCS):
		...
		self.dc = machine.Pin(aDC, machine.Pin.OUT, value=0)
		self.reset = machine.Pin(aReset, machine.Pin.OUT, value=0) if aReset != -1 else None
		self.cs = machine.Pin(aCS, machine.Pin.OUT, value=0) if aCS != -1 else None
	...
	# @micropython.native
	def initg(self, offset: bytearray = None):
