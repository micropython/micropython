...
// ugly, but using *p in interrupt handler doesn't work...
...
// returns position of first 1 in a number
// used to determine which gpio caused the interrupt
...
void Softuart_Init(Softuart *s, uint32_t baudrate) {
    ...
    if (!s->pin_rx.gpio_mux_name) {
        ...
        ETS_GPIO_INTR_ATTACH(Softuart_Intr_Handler, NULL);
...
void MP_FASTCODE(Softuart_Intr_Handler)(void *p) {
    ...
    if (gpio_id != 0xFF) {
        ...
        Softuart *s = _Softuart_GPIO_Instances[gpio_id];
        ...
        if (!level) {
            ...
            if (next != s->buffer.receive_buffer_head) {
                ...
                s->buffer.receive_buffer[s->buffer.receive_buffer_tail] = d;
...
uint8_t Softuart_Read(Softuart *s) {
    ...
    if (s->buffer.receive_buffer_head == s->buffer.receive_buffer_tail) {
        return 0; // TODO move return to argument
    }
