...
main()
{
    if [ "$SERIAL_NAME" = "" ]; then
    {
        ...
        echo -e "please open the \033[35mdownload_img.sh\033[0m file (e.g: \033[32mvim ./tools/download_img.sh\033[0m),"
        echo -e "and configure \033[36mSERIAL_NAME\033[0m to the serial port number you are using."
		exit $retval
    }
    fi

	./${DLIMG} -c ${SERIAL_NAME} -ds 2M -dl "${BUILD}/${TARGET}_gz.img" -ws 115200 -rs at
#	./${DLIMG} -c ${SERIAL_NAME} -ds 2M -dl "${BUILD}/${TARGET}_gz.img" -ws 115200 -rs at -sl str
...
#	./${DLIMG} -c ${SERIAL_NAME} -ds 2M -dl "${BUILD}/${TARGET}_gz.img" -rs rts
