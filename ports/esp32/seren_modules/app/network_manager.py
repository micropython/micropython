...
# import test_scripts.ntptime as ntptime
import gc
import urequests
...
class NetworkManager:
	...
	async def start_servers(self):
		...
		if not self.atcp.server:  # start the tcp connection server (Chataigne)
	...
	def ping_server(self):
		print("Pinging local network for HTTP servers on port 8000...")
		local_ip = self.wlan.ifconfig()[0]
		ip_parts = local_ip.split('.')
		found_update_server = False
		
		for i in range(1, 255):
			# self.nvs.wdt.feed()
			test_ip = f"{ip_parts[0]}.{ip_parts[1]}.{ip_parts[2]}.{i}"
			url = f"http://{test_ip}:8000/"
			print("Testing URL: ", url)
			try:
				resp = urequests.get(url, timeout=0.2)
				if resp.status_code == 200:
					print(f"Found update server at {test_ip}:8000")
					self.nvs.settings["ota_local_host"] = f"http://{test_ip}:8000/"
					found_update_server = True
					resp.close()
		...
		if not found_update_server:
			print("No local update server found on port 8000.")
			return False

	def update_locally(self, host=None, branch=None, model=None):
		if not host:
			print("Checking local OTA host for updates...")
			try:
				resp = urequests.get(self.nvs.settings["ota_local_host"], timeout=0.5)
				if resp.status_code == 200:
					print("Saved Local OTA host is reachable.")
					host = self.nvs.settings["ota_local_host"]
					resp.close()
			except Exception as e:
				print(f"Couldn't reach saved Local OTA host: {e}")
				host = self.ping_server()

			
		asyncio.create_task(self.update_device(host=host, branch=branch, model=model))
...
if __name__ == "__main__":
	import app.system as nvs_module
	import app.led as led_module
	import app.rotary as rotary_module
	import machine

	nvs = nvs_module.NVSManager()
	loop = asyncio.get_event_loop()
	rot = rotary_module.RotaryEncoder(nvs)
	led = led_module.LED(nvs)

	network_manager = NetworkManager(nvs, led, rot, loop)

	rot.set_dependencies(led=led, network_manager=network_manager, loop=loop)
	led.set_dependencies(rot=rot, loop=loop)

	async def main():
		# await network_manager.initialize_wifi()
		await network_manager.start_atcp()
		# Minimal test server
		import socket
		import ssl
		...
		try:
			...
			context.load_cert_chain(certfile='static/cert.der', keyfile='static/key.der')
			
			ssl_sock = context.wrap_socket(conn, 
										server_side=True,
										# cert='static/cert.der',
										# key='static/key.der',
										do_handshake_on_connect=True)
			
			# ssl_sock.do_handshake()  # This should fail with more specific error
