...
class AsyncMicReader:
    ...
    def __init__(self, nvs, led=None, espnow=None):
        ...
        self.defaults = {
            ...
            'mic_bit_depth': 32,
            'mic_buffer': 4096,  # 0 = no bug, 1 = bug
            'mic_decay_ms': 500,  # Default decay interval in milliseconds
            'mic_norm': 0.7,  # Default normalization factor
            'mic_interval_ms': 50,  # Default interval for monitoring in milliseconds
            'mic_max_val': 5000,  # Default threshold for max value
        }
        ...
        self.audio_in = I2S(
            ...
            mode=I2S.RX,
            bits=self.defaults["mic_bit_depth"],
            format=I2S.STEREO,
            rate=self.defaults["mic_sample_rate"],
            ibuf=self.defaults["mic_buffer"]
        )
        print("Initialized with I2S microphone")

        
        self.is_monitoring = False
        self.sound_task = False
    ...
    def toggle_mic_task(self, callback=None):
        ...
        if self.sound_task:
            ...
            self.sound_task = True
            self.led.start_mode(self.monitor_sound(callback))
