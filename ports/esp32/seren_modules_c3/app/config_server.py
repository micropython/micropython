...
class ConfigServer:
    def __init__(self, nvs, led, rot, atcp):
        ...
        self.buttons = {
            ...
            'System Controls': {
                ...
                'update_to_development_branch_(Experimental, might break your device) ': 'asyncio.create_task(network_manager.update_device(branch="dev"))',
                'update_to_BLE_branch_(Highly Experimental, Android users only!!) ': 'asyncio.create_task(network_manager.update_device(branch="BLE"))',
            }
        }

        # Add microphone toggle to compatible models
        self.buttons["Light Controls"]["toggle_microphone"] = 'nvs.mic.toggle_mic_task(led.b_f_c)' if nvs.mic else 'print("Microphone not available")'
        ...
        self.slider_met_vals = {
                "m_bri": ("Brightness",0.01,1.0,0.01,nvs.settings["led_m_bri"],"","Overall LED brightness"),
                "freq": ("Frequency",3.0,40.0,0.1,nvs.settings["led_freq"],"Hz","Light flicker frequency"),
                "duty": ("Duty Cycle",0.1,0.9,0.1,nvs.settings["led_duty"],"","On/off ratio"),
                "hue": ("Colour",1,360,1,nvs.settings["led_hue"],"","All the colours of the Rainbow"),
                "sat": ("Saturation",0.0,1.0,0.01,nvs.settings["led_sat"],"","Colour saturation"),
            }

        self.slider_metadata = self.generate_slider_metadata()
    ...
    def generate_slider_metadata(self, slider_selection=None):
        metadata = {}
        for key, values in self.slider_met_vals.items():
            metadata[key] = {
                "label": values[0],
                "min": values[1],
                "max": values[2],
