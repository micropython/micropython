...
_SETTING_COMMAND = const(0x7C)  # 124, |, the pipe character: The command to change settings: baud, lines, width,..
_SET_RGB_COMMAND = const(0x2B)  # 43, +, the plus character: command to set backlight RGB value
_ENABLE_SYSTEM_MESSAGE_DISPLAY = const(0x2E)  # 46, ., command to enable system messages being displayed
_DISABLE_SYSTEM_MESSAGE_DISPLAY = const(0x2F)  # 47, /, command to disable system messages being displayed
_ENABLE_SPLASH_DISPLAY = const(0x30)  # 48, 0, command to enable splash screen at power on
_DISABLE_SPLASH_DISPLAY = const(0x31)  # 49, 1, command to disable splash screen at power on
_SAVE_CURRENT_DISPLAY_AS_SPLASH = const(0x0A)  # 10, Ctrl+j, command to save current text on display as splash
_CLEAR_COMMAND = const(0x2D)  # 45, -, the dash character: command to clear and home the display
