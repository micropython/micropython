...
_DEFAULT_AUTH_MODE  = nvs.nvs_get_u8("system", "wifi.authmode")
_DEFAULT_PHASE_2    = nvs.nvs_get_u8("system", "wifi.phase2")
_DEFAULT_USERNAME   = nvs.nvs_getstr("system", "wifi.username")
_DEFAULT_ANON_IDENT = nvs.nvs_getstr("system", "wifi.anon_ident")

_AUTH_MODE_WPA2_ENTERPRISE = 5
...
def connect(*args):
	...
	_STA_IF.active(True)
	if len(args) == 0:
		if _DEFAULT_AUTH_MODE == _AUTH_MODE_WPA2_ENTERPRISE:
			wpa2enterprise.connect(_DEFAULT_SSID, _DEFAULT_ANON_IDENT, _DEFAULT_USERNAME, _DEFAULT_PASSWORD, _DEFAULT_PHASE_2)
		elif _DEFAULT_PASSWORD:
		...
		wpa2enterprise.connect(args[0], args[1], args[2], args[3], args[4])
	else:
