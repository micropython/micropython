...
1. Configure your board to use a boot loader by editing the mpconfigboard.mk
   ...
   The version and config options that mboot was built with are stored in at 
   the end of the mboot flash block. The length of this is section defaults
   to 64 bytes, but can be over-ridden by setting MBOOT_VER_LEN. If running
   ...
   over-ridden by setting MBOOT_VER. The version string is appended with 
   options mboot was built with - see ver.c for details. This can be prevented
   by setting MBOOT_VER_INC_OPTS to 0.
...
--------------------------------------
The following python code retrieves the version of mboot currently on the device:
```python
import stm

def get_mboot_version(include_opts=True):
    MBOOT_BASE = const(0x0800_0000)
    MBOOT_LEN = const(32 * 1024)  # this will need to updated if mboot is a different size
    VER_LEN = const(64)  # this will need to be updated if the length/location of version string changes
    base = const(MBOOT_BASE + MBOOT_LEN - VER_LEN)
    s = ''
    for i in range(VER_LEN):
        c = stm.mem8[base + i]
        if c == 0xFF or c == 0:  # have hit empty flash or a null-terminator
            break
        s += chr(c)
    if s.startswith('mboot-v'):
        if include_opts:
            return s
        else
