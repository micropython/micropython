...
#if MICROPY_PY_TSTRINGS
static void *coverage_alloc_fn(void *ctx, size_t n) {
    (void)ctx;
    return m_malloc(n);
}
#endif
...
static mp_obj_t extra_coverage(void) {
    ...
    #if MICROPY_PY_TSTRINGS
    {
        mp_printf(&mp_plat_print, "# t-strings\n");

        extern mp_parse_node_t parse_tstring_expression(void *alloc_ctx, mp_parse_allocator_t allocator, const char *expr, size_t len);

        nlr_buf_t nlr_empty;
        if (nlr_push(&nlr_empty) == 0) {
            parse_tstring_expression(NULL, coverage_alloc_fn, "", 0);
            nlr_pop();
            mp_printf(&mp_plat_print, "ERROR: Empty expression should have raised SyntaxError\n");
        } else {
            mp_obj_print_exception(&mp_plat_print, MP_OBJ_FROM_PTR(nlr_empty.ret_val));
        }

        nlr_buf_t nlr_ws;
        if (nlr_push(&nlr_ws) == 0) {
            parse_tstring_expression(NULL, coverage_alloc_fn, "   ", 3);
            nlr_pop();
            mp_printf(&mp_plat_print, "ERROR: Whitespace-only expression should have raised SyntaxError\n");
        } else {
            mp_obj_print_exception(&mp_plat_print, MP_OBJ_FROM_PTR(nlr_ws.ret_val));
        }
...

        {
            ...
            if (nlr_push(&nlr_size) == 0) {
                ...
                char code[1060];
                snprintf(code, sizeof(code), "t\"%s\"", large_str);

                mp_lexer_t *lex = mp_lexer_new_from_str_len(MP_QSTR__lt_test_gt_, code, strlen(code), 0);
                mp_parse_tree_t parse_tree = mp_parse(lex, MP_PARSE_FILE_INPUT);
                mp_obj_t module = mp_compile(&parse_tree, MP_QSTR__lt_test_gt_, false);
                mp_call_function_0(module);

                nlr_pop();
                mp_printf(&mp_plat_print, "ERROR: Should have raised ValueError\n");
            } else {
                mp_obj_print_exception(&mp_plat_print, MP_OBJ_FROM_PTR(nlr_size.ret_val));
            }

            nlr_buf_t nlr_interp;
            if (nlr_push(&nlr_interp) == 0) {
                char code[1000];
                strcpy(code, "t\"");
                for (int i = 0; i < 130; i++) {
                    strcat(code, "{x}");
                ...
                nlr_pop();
                mp_printf(&mp_plat_print, "ERROR: Should have raised SyntaxError\n");
            } else {
                mp_obj_print_exception(&mp_plat_print, MP_OBJ_FROM_PTR(nlr_interp.ret_val));
            }
        }
        ...
        {
            mp_printf(&mp_plat_print, "# Testing escaped braces in format spec\n");
        ...
        {
            mp_printf(&mp_plat_print, "# Testing conversion after format spec\n");
            const char *test_code = "t'{x:.2f!r}'";
            ...
            if (nlr_push(&nlr) == 0) {
                ...
                nlr_pop();
                mp_printf(&mp_plat_print, "Conversion after format spec: OK\n");
            } else {
                mp_obj_print_exception(&mp_plat_print, MP_OBJ_FROM_PTR(nlr.ret_val));
            }
        }
        ...
        {
            mp_printf(&mp_plat_print, "# Testing empty format spec\n");
            const char *test_code = "t'{x:}'";
            ...
            if (nlr_push(&nlr) == 0) {
                ...
                nlr_pop();
                mp_printf(&mp_plat_print, "Empty format spec: OK\n");
            } else {
                mp_obj_print_exception(&mp_plat_print, MP_OBJ_FROM_PTR(nlr.ret_val));
            }
        }
        ...
        {
            mp_printf(&mp_plat_print, "# lexer fstring_args < 3 chars\n");
            mp_lexer_t *lex = mp_lexer_new_from_str_len(MP_QSTR__lt_test_gt_, "", 0, 0);
            if (lex) {
                vstr_clear(&lex->fstring_args);
                vstr_init(&lex->fstring_args, 2);
                vstr_add_byte(&lex->fstring_args, 'x');
                lex->fstring_args_idx = 0;
                mp_lexer_to_next(lex);
                mp_printf(&mp_plat_print, "Defensive code path tested\n");
                mp_lexer_free(lex);
            }
        }
        #endif
        ...
        #if MICROPY_PY_TSTRINGS
        {
            mp_printf(&mp_plat_print, "# objtype.c Template/str errors\n");
            mp_obj_template_t *tmpl = m_new_obj(mp_obj_template_t);
            tmpl->base.type = &mp_type_template;
            tmpl->strings = mp_obj_new_tuple(1, (mp_obj_t[]) {MP_OBJ_NEW_QSTR(MP_QSTR_hello)});
            tmpl->interpolations = mp_const_empty_tuple;
            mp_obj_t tmpl_obj = MP_OBJ_FROM_PTR(tmpl);
            mp_obj_t str_obj = MP_OBJ_NEW_QSTR(MP_QSTR_world);

            nlr_buf_t nlr_tmpl_str;
            if (nlr_push(&nlr_tmpl_str) == 0) {
                mp_binary_op(MP_BINARY_OP_ADD, tmpl_obj, str_obj);
                nlr_pop();
            } else {
                mp_obj_print_exception(&mp_plat_print, MP_OBJ_FROM_PTR(nlr_tmpl_str.ret_val));
            }

            nlr_buf_t nlr_str_tmpl;
            if (nlr_push(&nlr_str_tmpl) == 0) {
                mp_binary_op(MP_BINARY_OP_ADD, str_obj, tmpl_obj);
                nlr_pop();
            } else {
                mp_obj_print_exception(&mp_plat_print, MP_OBJ_FROM_PTR(nlr_str_tmpl.ret_val));
            }
        }
        #endif
