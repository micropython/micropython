...
const adc_cfg_t g_adc0_cfg = { .unit = 0, .mode = ADC_MODE_SINGLE_SCAN,
                               .resolution = ADC_RESOLUTION_14_BIT, .alignment =
...
const lpm_cfg_t g_lpm0_cfg = { .low_power_mode = LPM_MODE_SLEEP,
                               ...
                               .standby_wake_sources = LPM_STANDBY_WAKE_SOURCE_RTCALM
                                   | (lpm_standby_wake_source_t)0, .snooze_request_source =
                                   LPM_SNOOZE_REQUEST_RXD0_FALLING, .snooze_end_sources =
...
const external_irq_cfg_t g_external_irq9_cfg = { .channel = 9, .trigger =
                                                     ...
                                                     EXTERNAL_IRQ_PCLK_DIV_BY_64, .p_callback = callback_icu, .p_context =
                                                     NULL, .p_extend = NULL, .ipl = (12),
...
{ .mode = TIMER_MODE_PERIODIC,
  /* Actual period: 0.002730666666666667 seconds. Actual duty: 50%. */ .period_counts =
...
/* Baud rate calculated with 0.160% error. */ .abcse = 0, .abcs = 0, .bgdm = 1,
    .cks = 0, .brr = 25, .mddr = (uint8_t)256, .brme = false
};
...
const sci_uart_extended_cfg_t g_uart9_cfg_extend = {
    .clock = SCI_UART_CLOCK_INT, .rx_edge_start =
        ...
        &g_uart9_baud_setting, .uart_mode = UART_MODE_RS232,
...
const sci_uart_extended_cfg_t g_uart1_cfg_extend = {
    .clock = SCI_UART_CLOCK_INT, .rx_edge_start =
        ...
        SCI_UART_NOISE_CANCELLATION_DISABLE, .rx_fifo_trigger =
        SCI_UART_RX_FIFO_TRIGGER_MAX, .p_baud_setting =
        &g_uart1_baud_setting, .uart_mode = UART_MODE_RS232,
    .ctsrts_en = SCI_UART_CTSRTS_RTS_OUTPUT,
    #if 0
    .flow_control_pin = BSP_IO_PORT_00_PIN_00,
    #else
