...
void dcd_init(uint8_t rhport)
{
    ...
    // write gevntadr with addr to the event buffer (remember addr map)
...
bool dcd_edpt_xfer(uint8_t rhport, uint8_t ep_addr, uint8_t * buffer, uint16_t total_bytes)
{
    ...
    switch (ep) {
        ...
        case 1: { // CONTROL IN
            ...
            // thus the only STATUS IN we produce is for data-less control txn
            uint8_t type = TRBCTL_CTL_STAT2;
...
static void _dcd_handle_depevt(uint8_t ep, uint8_t evt, uint8_t sts, uint16_t par)
{
    ...
    switch (evt) {
        case DEPEVT_XFERCOMPLETE: {
            ...
            if (0 == ep) { // [TODO] validate EP stage (CONTROL_STAGE_SETUP)
        ...
        case DEPEVT_XFERNOTREADY: {
            ...
            // this event is received when host tries to send/receive data but
            // there's no trb set up for it
            if (0 == ep) {
                // with TinyUSB implementing this isn't really necessary
                // the stack will control the flow of the transactions by itself
            }
        } break;
...
static uint8_t _dcd_start_xfer(uint8_t ep, void* buf, uint32_t size, uint8_t type)
{
    ...
    _xfer_trb[ep][0] = (uint32_t) LocalToGlobal(buf);
    _xfer_trb[ep][1] = 0;
    _xfer_trb[ep][2] = size;
