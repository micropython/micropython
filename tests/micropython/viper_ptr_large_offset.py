...
@micropython.viper
def load8_neg(src: ptr8) -> int:
    # small negative constant offset (uses the unscaled-offset form)
    return src[-1]


@micropython.viper
def load8_neg_large(src: ptr8) -> int:
    # negative constant offset beyond the unscaled range
    return src[-300]


@micropython.viper
def store8_neg(dest: ptr8, val: int):
    dest[-1] = val


@micropython.viper
def load32_neg(src: ptr32) -> int:
    return src[-1]
...
# negative constant offsets: pass a memoryview slice so that src[-1] etc.
# stay within the underlying buffer
mv8 = memoryview(buf8)
print(hex(load8_neg(mv8[100:])))
print(hex(load8_neg_large(mv8[400:])))
store8_neg(mv8[200:], 0x5A)
print(hex(buf8[199]))

mv32 = memoryview(buf32)
print(hex(load32_neg(mv32[8:])))
