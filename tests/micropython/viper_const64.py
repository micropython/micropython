# Test large integer constants in viper mode.
#
# Viper's "int" type is the target's native machine integer, so constants wider
# than the machine word are not representable and raise ViperTypeError.  The
# functions below are compiled at run time with exec() so that 32-bit targets
# (and mpy-cross, which cannot compile such constants at all) skip this test
