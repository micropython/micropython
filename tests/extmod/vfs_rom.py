...
class TestEmptyROMFS(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.empty_romfs = make_romfs(())

    def test(self):
        self.assertEqual(self.empty_romfs, bytes([0x80 | ord("R"), 0x80 | ord("M"), ord("1"), 0]))
        fs = vfs.VfsRom(self.empty_romfs)
