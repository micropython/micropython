...
# 19. Unaligned offset for ldr (must be a multiple of 8)
try:
    exec("@micropython.asm_aarch64\ndef f(x0):\n    ldr(x0, [x0, 4])\n")
except SyntaxError:
    print("SyntaxError: unaligned offset")

# 20. Immediate for add that is neither 12-bit nor 12-bit shifted left by 12
try:
    exec("@micropython.asm_aarch64\ndef f(x0):\n    add(x0, x0, 0x1001)\n")
except SyntaxError:
    print("SyntaxError: bad shifted immediate")

# 21. Register-offset addressing (a register where an immediate is required)
try:
    exec("@micropython.asm_aarch64\ndef f(x0, x1):\n    ldr(x0, [x0, x1])\n")
except SyntaxError:
    print("SyntaxError: register offset")

# 22. Immediate for cmp that is neither 12-bit nor 12-bit shifted left by 12
try:
    exec("@micropython.asm_aarch64\ndef f(x0):\n    cmp(x0, 0x1001)\n")
except SyntaxError:
    print("SyntaxError: cmp bad shifted immediate")

# 23. Immediate for sub that is neither 12-bit nor 12-bit shifted left by 12
try:
    exec("@micropython.asm_aarch64\ndef f(x0):\n    sub(x0, x0, 0x1001)\n")
except SyntaxError:
    print("SyntaxError: sub bad shifted immediate")

# 24. push without a register list
try:
    exec("@micropython.asm_aarch64\ndef f(x0):\n    push(x0)\n")
except SyntaxError:
    print("SyntaxError: push expects reglist")

# 25. Non-register in a push register list
try:
    exec("@micropython.asm_aarch64\ndef f(x0):\n    push({x0, 5})\n")
except SyntaxError:
    print("SyntaxError: push expects register")

# 26. Unknown instruction with no arguments
try:
    exec("@micropython.asm_aarch64\ndef f():\n    xyz()\n")
except SyntaxError:
    print("SyntaxError: unknown 0-arg op")

# 27. Unknown two-letter condition code in a 3-character branch
try:
    exec("@micropython.asm_aarch64\ndef f(x0):\n    bzz(x0)\n")
except SyntaxError:
    print("SyntaxError: unknown condition code")
