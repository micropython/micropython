...
# correctly go into the various error states, and can then recover.
#
...
# State result list indexes, as constants
_IDX_TEC = const(0)
_IDX_REC = const(1)
...
def instance1():
    ...
    # queue another message. This will keep trying to send until we go back to ERROR_PASSIVE
    idx = can.send(ID, b"YETMORE")
    ...
    # To recover this time, tell the receiver to go back to a valid baud rate
    multitest.broadcast("fix baud")
    multitest.wait("fixed baud")

    idx_more = can.send(ID, b"MOREMORE")
    time.sleep_ms(50)  # message should send during this window
