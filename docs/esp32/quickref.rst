...
- ``ref_clk`` - :class:`machine.Pin` object (or integer) specifying the pin used
  for the EMAC ``ref_clk`` signal. If not specified, the compile-time default is
  used (typically GPIO 0, but a board-specific default may be set for a board with
  Ethernet.
- ``ref_clk_mode`` - Defines whether the EMAC ``ref_clk`` pin of the ESP32
  ...
  ``machine.Pin.OUT``. If not specified, the compile-time default is used
  (typically input, but may also have a board-specific default set.)
