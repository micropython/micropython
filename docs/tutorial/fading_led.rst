...
Code
====
By examining the `schematics <http://micropython.org/static/doc/PYBv10b.pdf>`_
, we see that ``X1`` is connected to channel 1 of timer 5 (``TIM5_CH1``). Therefore we will first create a ``Timer`` object for timer 5, then create a ``TimerChannel`` object for channel 1::
