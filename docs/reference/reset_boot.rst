...
Booting from hard reset happens when the device is powered on, if Python code
executes :func:`machine.reset()`, or if a physical Reset button is pressed on
...
- Any peripherals configured using the :doc:`machine module </library/machine>`,
  including :doc:`machine.Pin </library/machine.Pin>` mode settings.
- Bluetooth.
...
- The device's :doc:`REPL <repl>` is continuous before and after soft reset,
...
- :doc:`RTC </library/machine.RTC>` setting of the current time is retained
  after soft reset.
...
<filesystem>` using :doc:`mpremote <mpremote>` or :doc:`pyboard.py
<pyboard.py>`.
...
The REPL continues executing until Python code triggers a reset.
...
It is rare but possible for MicroPython to become unresponsive during startup,
for example:
