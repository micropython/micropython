...
The PSOC™ Edge port offers ``LFS2`` (LittleFS v2) filesystem, implemented in :ref:`MicroPython <filesystem>`. The ``LFS2`` filesystem is selected as the default given its stability and reliability. The filesystem is located on the external QSPI flash, which has a capacity of 64 MB. 
...
The MicroPython port for PSOC™ Edge can be detected by the `Thonny IDE <https://thonny.org/>`_ when the ``MicroPython (generic)`` option is selected at the bottom right corner, as shown. Additionally, the filesystem is detected by the IDE, as shown in the lower left column. 
