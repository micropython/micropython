...
output; tests with a ``.exp`` file use that instead, and unittest-based
tests are judged by their printed test summary.
...
For unittest-based tests the result line also shows the number of test
cases run, e.g. ``pass  extmod/my_module_test.py (unittest: 3 ran)``.
...
These three options perform their action on the existing ``results/``
directory and exit without running any tests.
...
``run-perfbench.py`` runs standardised benchmarks. The two positional
arguments are approximate CPU MHz and heap KB of the target; each benchmark
uses them to select a suitable iteration count and data size.
...
Specific benchmarks can be selected:
...
``run-multitests.py``. Each ``-t`` argument provides one instance; if a test
needs more instances than were given, extra unix-port instances are started
automatically.
