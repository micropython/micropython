...
When writing tests for MicroPython-specific features (that cannot run on CPython), 
it is generally preferable to use ``unittest``-based tests rather than ``.exp`` files. 
The ``unittest`` framework provides better structure, clearer error messages, and 
more maintainable tests. The main exception is for tests that run against native 
modules, where the native module test framework currently does not support ``unittest``.
