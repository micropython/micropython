...
Driver for SSD1306 monochrome OLED displays.
...
SPI interface::
    ...
    # hardware SPI
    spi = SPI(1) # scl=14, mosi=13 (sda), miso=12 (unused)
    dc = Pin(4, Pin.OUT)  # data/command
    rst = Pin(5, Pin.OUT) # reset
    cs = Pin(15, Pin.OUT) # chip select, some modules do not have a pin for this
    display = ssd1306.SSD1306_SPI(128, 64, spi, dc, rst, cs)

    # software SPI
    scl = Pin(14, Pin.OUT) # serial clock
    sda = Pin(13, Pin.OUT) # serial data
    miso = Pin(12) # unused
    spi = SPI(baudrate=500000, polarity=0, phase=0, sck=scl, mosi=sda, miso=miso)
    dc = Pin(4, Pin.OUT)  # data/command
    rst = Pin(5, Pin.OUT) # reset
    cs = Pin(15, Pin.OUT) # chip select, some modules do not have a pin for this
    display = ssd1306.SSD1306_SPI(128, 64, spi, dc, rst, cs)
...
Examples::
    ...
    # MicroPython logo and some text
    display.fill(0)
