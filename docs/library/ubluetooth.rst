...
-------------------
    ...
    When bonded, a device is able to resolve an RPA address from another device
    based on the stored IRK. To support bonding, an application must implement
    the ``_IRQ_GET_SECRET`` and ``_IRQ_SET_SECRET`` events.
