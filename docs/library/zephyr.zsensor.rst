...
.. method:: Sensor.attr_set(sensor_channel, channel_attribute, val1, val2=0)

    Set the given channel's attribute to the given value.
    If ``val1`` is not adequate to represent the integral value, use ``val1`` for the value's
    integer part and ``val2`` for the value's fractional part in millionths.

    Returns ``None`` if successful, or raises ``OSError``.


.. method:: Sensor.attr_get(sensor_channel, channel_attribute)

   Gets the value of the given channel's attribute as an integer tuple.

   Many sensors do not support this method.
