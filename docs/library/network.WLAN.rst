...
.. method:: WLAN.csi_read()
   ...
   * **22 - agc_gain** (int): AGC (Automatic Gain Control) value (ESP32-S3/C3/C5/C6 only, 0 on other platforms)
   * **23 - fft_gain** (int): FFT scaling gain value (ESP32-S3/C3/C5/C6 only, 0 on other platforms)
...
.. method:: WLAN.csi_force_gain(agc, fft)
   ...
   * **agc** (int or None): AGC gain value (0-255), or ``None`` to re-enable automatic control
   * **fft** (int or None): FFT gain value (0-255), or ``None`` to re-enable automatic control
   ...
   **Usage:**
   ...
   The recommended approach is to collect gain statistics from the first ~300 packets
   after Wi-Fi connection, calculate the average, and then lock the gain to those values::

      # Check if gain lock is supported
      if wlan.csi_gain_lock_supported():
          # Collect gain statistics from first 100 packets
          import time
          fft_sum, agc_sum, count = 0, 0, 0
          while count < 100:
              frame = wlan.csi_read()
              if frame:
                  agc_sum += frame[22]  # agc_gain
                  fft_sum += frame[23]  # fft_gain
                  count += 1
              else:
                  time.sleep_ms(10)  # Wait for more frames

          # Lock gain to average values
          avg_agc = agc_sum // count
          avg_fft = fft_sum // count
          wlan.csi_force_gain(avg_agc, avg_fft)
          print(f"Gain locked: AGC={avg_agc}, FFT={avg_fft}")
