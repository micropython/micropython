...
.. classmethod:: Partition.mark_app_invalid_rollback_and_reboot()
    ...
    If the rollback is successful, the device will reset.  If the flash does not have 
...
.. method:: Partition.app_state()
    ...
    Returns the app state of a valid OTA partition as an integer. Compare the return
    value with the ``APP_STATE_*`` constants:

    - ``APP_STATE_NEW``: Monitor the first boot. In bootloader this state is changed
      to ``APP_STATE_PENDING_VERIFY``.
    - ``APP_STATE_PENDING_VERIFY``: First boot for this app. If this state persists
      during second boot, then it will be changed to ``APP_STATE_ABORTED``.
    - ``APP_STATE_VALID``: App was confirmed as workable. App can boot and work
      without limits.
    - ``APP_STATE_INVALID``: App was confirmed as non-workable. This app will not
      be selected to boot at all.
    - ``APP_STATE_ABORTED``: App could not be confirmed as workable or non-workable.
      In bootloader ``APP_STATE_PENDING_VERIFY`` state will be changed to
      ``APP_STATE_ABORTED``. This app will not be selected to boot at all.
    - ``APP_STATE_UNDEFINED``: App can boot and work without limits (rollback not
      enabled).

    Example::

        cur = Partition(Partition.RUNNING)
        if cur.app_state() == Partition.APP_STATE_VALID:
            print("Current app is valid")

    One of the following OSError can be raised:

    - ``ESP_ERR_NOT_SUPPORTED``: Partition is not OTA.
    - ``ESP_ERR_NOT_FOUND``: Partition table does not have otadata or state was not
      found for given partition. 
...
.. data:: Partition.APP_STATE_NEW
    ...
    Constants for interpreting the return value of `Partition.app_state()`.
