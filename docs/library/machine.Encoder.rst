...
.. method:: Encoder.init(phase_a, phase_b, *, ...)
   ...
   - *max*\=value. Specify the upper counting range. The position counter will count up
...
.. method:: Encoder.irq(trigger=event, value=nnn, handler=handler, hard=False)
   ...
   *event* may be:
    - Encoder.IRQ_HOME Triggered with a transition at the *home* input.
