...
Here is described a basics, commons for hadware counters based MicroPython ports:

* :ref:`ESP32.Encoder <pcnt.Encoder>`
* mimxrt.Encoder
* STM32.Encoder
* and an IRQ-based encoders `Incremental encoders <https://github.com/peterhinch/micropython-samples/blob/master/encoders/ENCODERS.md>`_

Example usage::

    from machine import Pin, Encoder

    qe = Encoder(Pin(0), Pin(1))  # create Quadrature Encoder object and start to count input pulses
    qe.value()                    # get current raw encoder value
    qe.set_value(0)               # set raw encoder value to 0
    qe.deinit()                   # turn off the Qudrature encoder

    print(qe)                     # show the Encoder object properties
...
.. class:: Encoder(input_a, input_b, \*, keyword_arguments)
    ...
    Keyword arguments:
      ...
      - *init*\=value. Sets the init value. The default value is 0.
...
.. method:: Encoder.position([value])
   ...
   With a single *value* argument the position counter is set to that value.
   Pseudocode is::
