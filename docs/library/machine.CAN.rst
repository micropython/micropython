...
.. method:: CAN.init(bitrate, mode=CAN.MODE_NORMAL, sample_point=75, sjw=1, tseg1=None, tseg2=None, auto_retransmit=True)
   ...
   - ``mode`` is one of the values show under `can-modes`, indicating the
...
.. method:: CAN.init_fd(bitrate, sample_point=None, sjw=None, tseg1=None, tseg2=None)
   ...
   .. note:: This method is not currently implemented for any CAN controller
             hardware.
   ...
   .. note:: To avoid the controller temporarily becoming active on the bus
             ...
             with ``mode`` set to ``CAN.MODE_SLEEP``, then call this function to
...
.. method:: CAN.set_filters(filters)
   ...
   Set receive filters in the CAN controller. ``filters`` should be ``None`` to
   disable all message receiving, or otherwise an iterable where each item is a
   tuple or list with three elements:
  ...
  Some CAN controllers require each filter to be associated with only one receive FIFO.
  In these cases, the filters in the argument are allocated round-robin to the available
  FIFOs. This driver does not distinguish between FIFOs in the receive IRQ.
...
.. method:: CAN.send(id, data, flags=0)
   ...
   value is the hardware buffer index where the message is queued to send.
...
.. method:: CAN.recv(arg=None)
   ...
   Otherwise, it returns a list with 4 elements:
   ...
   - Index 3 is an integer with zero or more of the bits defined in
     ...
     receiving CAN messages. These flags are reset each time this function
     returns.
...
.. method:: CAN.irq(handler=None, trigger=0, hard=False)
   ...
   events flagged in *trigger* has occurred.
     ...
     - *trigger* configures the event(s) which can generate an interrupt.
       ...
       - `CAN.IRQ_STATE` event occurs when the CAN controller changes into a more
         severe error state. Call :func:`CAN.get_state()` to get the updated state.
...
.. method:: CAN.cancel_send(index)
   ...
   *index* is the index of a hardware transmit buffer. It should be an integer
    in the range ``0`` to ``CAN.TX_QUEUE_LEN`` (exclusive). Generally this will
    be a value previously returned by :func:`CAN.send()`.
...
.. method:: CAN.restart()
   ...
   The alternative to calling `CAN.restart()` is to call `CAN.deinit()` and
   then `CAN.init()` to fully reset the controller's entire internal state.
