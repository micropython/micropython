...
.. method:: Counter.init(src, *, ...)
   ...
   - *max*\=value. Specify the upper counting range. The counter will count up
     from a *min* init value up to *max* - 1, then roll over to the init value and
     ...
     both *max* and *min* to 0.In that case the range is the hardware's counter range.
     *(Supported by MIMXRT and the ESP32 PCNT module)*
 
   - *min*\=value. Specify the lower counting range.
     *(Supported by MIMXRT and the ESP32 PCNT module)*
   ...
   - *home*\=value. A Pin specifier telling to which pin the home pulse is connected.
     At a rising slope of the home pulse the counter is set to the init
     value, but the cycles counter is not changed. A *value* of *None* disables the home input.
     *(Supported on MIMXRT)*
 
   - *match*\=value. Set the counter value at which the interrupt IRQ_MATCH shall trigger.
...
.. method:: Counter.irq(trigger=event, value=nnn, handler=handler, hard=False)
   ...
   *event* may be:
    - Counter.IRQ_HOME Triggered with a transition at the *home* input.
