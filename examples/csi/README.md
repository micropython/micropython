...
mpremote connect /dev/cu.usbmodem11401 run examples/csi/csi_turbolence_monitor.py
```
...
#               secondary_channel, ant, sig_len, rx_state)
frame = wlan.csi_read()
if frame:
...
Each frame returned by `csi_read()` is a tuple with 22 fields (in order):
...
- **Verify firmware**: Make sure CSI is enabled in build configuration
  - If using ``ESP32_CSI_C6`` or ``ESP32_CSI_S3`` boards, CSI is pre-enabled
...
- **Recommended board**: Use ``ESP32_CSI_C6`` board for pre-configured CSI support
...
- **Recommended board**: Use ``ESP32_CSI_S3`` board for ESP32-S3 with pre-configured CSI support
