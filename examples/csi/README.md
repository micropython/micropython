...
**Pre-configured boards**: The ``ESP32_GENERIC_C3``, ``ESP32_GENERIC_C5``, ``ESP32_GENERIC_C6`` and ``ESP32_GENERIC_S3`` boards with ``CSI`` variant have
...
- **Verify firmware**: Make sure CSI is enabled in build configuration
  - If using ``ESP32_GENERIC_C3``, ``ESP32_GENERIC_C5``, ``ESP32_GENERIC_C6`` or ``ESP32_GENERIC_S3`` with ``CSI`` variant, CSI is pre-enabled
...
- **Recommended board**: Use ``ESP32_GENERIC_C5`` or ``ESP32_GENERIC_C6`` board with ``CSI`` variant
