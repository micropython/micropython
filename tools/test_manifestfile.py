#!/usr/bin/env python3
#
# Unit tests for manifestfile.py. Run with `python3 tools/test_manifestfile.py`.
...
# Make manifestfile importable when run from the repo root or from tools/.
...
class TestCModule(unittest.TestCase):
    ...
    def test_unresolved_path_var(self):
        with self.assertRaisesRegex(ManifestFileError, r"Unresolved variable in path"):
