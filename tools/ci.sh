...
function ci_wasm_setup {
    git clone https://github.com/emscripten-core/emsdk.git
    (cd emsdk && ./emsdk install latest && ./emsdk activate latest)
}

function ci_wasm_build {
    source emsdk/emsdk_env.sh
    make ${MAKEOPTS} -C ports/wasm
}

function ci_wasm_run_tests {
    # This port is very slow at running, so only run a few of the tests.
    (cd tests && MICROPY_MICROPYTHON=../ports/wasm/node_run.sh ./run-tests.py -j1 basics/builtin_*.py)
}
