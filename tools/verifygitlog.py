...
def verify(sha):
    ...
    for line in git_log("%ae%n%ce", sha, "-n1"):
        ...
        if "noreply" in line:
            error("Unwanted email adres: " + line)
    ...
    if not re.match(r"^[^!]+: [A-Z]+.+ .+\.$", subject_line):
        error("Incorrect subject line format: " + subject_line)
    if len(subject_line) >= 73:
        error("Subject line should be < 73 characters: " + subject_line)
    ...
    for line in raw_body[2:]:
        if len(line) >= 76:
            error("Message lines should be < 76 characters: " + line)
