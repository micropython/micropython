...
typedef enum {
    MP_CAN_MODE_NORMAL,
    MP_CAN_MODE_SLEEP, // TODO: do we need this?
...
// Index of an operation is encoded into the irq().flags() response int
...
// These 'unsigned' values appear in the same order
// as the result of CAN.get_counters()
typedef struct {
    unsigned tec;
    unsigned rec;
    unsigned num_warning; // Number of "Error Warning" transitions
    unsigned num_passive; // Number of "Error Passive" transitions
    unsigned num_bus_off; // Number of "Bus-Off" transitions
    unsigned tx_pending;
    unsigned rx_pending;
